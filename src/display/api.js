...
PDFJS.getDocument = function getDocument(src,
  ...
  worker.promise.then(function () {
    ...
    return worker.fetchDocument(params, rangeTransport, docId).then(
        function (workerId) {
      if (task.destroyed) {
        throw new Error('Loading aborted');
      }
...
var PDFDocumentLoadingTask = (function PDFDocumentLoadingTaskClosure() {
  ...
  PDFDocumentLoadingTask.prototype =
    ...
    destroy: function () {
      ...
      var workerDestroyed = transportDestroyed.then(function () {
        this._transport = null;
        if (this._worker) {
...
var PDFWorker = (function PDFWorkerClosure() {
  var fakeWorkerReady = null;
  var nextFakeWorkerId = 0;

  // Loads worker code into main thread and setups up the port we will be using
  // in MessageHandler instances.
  function setupFakeWorkerGlobal() {
    if (fakeWorkerReady) {
      return fakeWorkerReady;
    }

    fakeWorkerReady = new Promise(function (resolve) {
...
//#endif
     ...
     * @ignore
     */
    fetchDocument: function PDFWorker_fetchDocument(source,
                                                    pdfDataRangeTransport,
                                                    docId) {
      if (this.destroyed) {
      ...
      return this._messageHandler.sendWithPromise('GetDocRequest', {
        docId: docId,
        source: source,
    ...
    _setupFakeWorker: function PDFWorker_setupFakeWorker() {
      setupFakeWorkerGlobal().then(function (port) {
        if (this.destroyed) {
          this._readyCapability.reject(new Error('Worker was destroyed'));
          return;
        }
